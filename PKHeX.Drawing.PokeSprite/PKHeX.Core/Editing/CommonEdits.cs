...
public static class CommonEdits
{
    ...
    public static uint GetComplicatedEC(ISpeciesForm pk, char option = ' ')
    {
        ...
        uint mod = 1, noise = 0;
        if (pk.Species is >= (int)Species.개무소 and <= (int)Species.독케일)
        {
            mod = 10;
            bool lower = option is '0' or 'B' or 'S' || WurmpleUtil.GetWurmpleEvoGroup(pk.Species) == 0;
            noise = (lower ? 0u : 5u) + (uint)rng.Next(0, 5);
        }
        else if (pk.Species is (int)Species.노고치 or (int)Species.노고고치 or (int)Species.두리쥐 or (int)Species.파밀리쥐)
        {
            mod = 100;
            noise = option switch
            {
                '0' or '3' => 0u,
                _ when pk.Species is (int)Species.노고고치 && pk.Form == 1 => 0, // 3 Segment
                _ when pk.Species is (int)Species.파밀리쥐 && pk.Form == 0 => 0, // Family of 3
