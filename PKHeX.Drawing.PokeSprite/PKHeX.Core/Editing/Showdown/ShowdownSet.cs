...
public sealed class ShowdownSet : IBattleTemplate
{
    ...
    private static ReadOnlySpan<ushort> DashedSpecies => new ushort[]
    {
        (int)니드런F, (int)니드런M,
        (int)칠색조,
        (int)짜랑꼬, (int)짜랑고우, (int)짜랑고우거,
        (int)딩루, (int)파오젠, (int)총지엔, (int)위유이,
    };
    ...
    private void SanitizeResult()
    {
        ...
        if (Species is (int)냐오닉스 or (int)에써르 or (int)대쓰여너 or (int)퍼퓨돈)
            ReviseGenderedForms();
    }
    ...
    {
        if (!identifier.EndsWith("성격", StringComparison.OrdinalIgnoreCase))
            return false;
    ...
    {
        "특성"       => (Ability = StringUtil.FindIndexIgnoreCase(Strings.abilitylist, value)) >= 0,
        "성격"        => (Nature  = StringUtil.FindIndexIgnoreCase(Strings.natures    , value)) >= 0,
        "이로치"         => Shiny         = StringUtil.IsMatchIgnoreCase("Yes", value),
        "기간타맥스"    => CanGigantamax = StringUtil.IsMatchIgnoreCase("Yes", value),
        "친밀도"    => ParseFriendship(value),
        "노력치"           => ParseLineEVs(value),
        "개체값"           => ParseLineIVs(value),
        "레벨"         => ParseLevel(value),
        "다이맥스 레벨" => ParseDynamax(value),
        "테라타입"     => ParseTeraType(value),
        _ => false,
    };
    ...
    public List<string> GetSetLines()
    {
        ...
        if (Context == EntityContext.Gen9 && TeraType != MoveType.Any && (uint)TeraType < Strings.Types.Count)
            result.Add($"테라타입: {Strings.Types[(int)TeraType]}");
        if (Level != 100)
            result.Add($"레벨: {Level}");
        if (Shiny)
            result.Add("이로치: Yes");
    ...
    private string GetStringFirstLine(string form)
    {
        ...
        else if (Species == (int)니드런M)
            specForm = specForm.Replace("♂", "-M");
        else if (Species == (int)니드런F)
