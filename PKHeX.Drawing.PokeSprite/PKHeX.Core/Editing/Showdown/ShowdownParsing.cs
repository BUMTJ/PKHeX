...
public static class ShowdownParsing
{
    ...
    public static string GetShowdownFormName(ushort species, string form)
    {
        ...
        return species switch
        {
            (int)배쓰나이 when form is "Blue"           => "Blue-Striped",
            (int)비비용 when form is "Poké Ball"      => "Pokeball",
            (int)지가르데                                => form.Replace("-C", string.Empty).Replace("50%", string.Empty),
            (int)메테노   when form.StartsWith("M-", StringComparison.OrdinalIgnoreCase)  => MiniorFormName,
            (int)메테노                                 => form.Replace("C-", string.Empty),
            (int)네크로즈마 when form is "Dusk"           => $"{form}-Mane",
            (int)네크로즈마 when form is "Dawn"           => $"{form}-Wings",
            (int)포트데스 or (int)데인차           => form == "Antique" ? form : string.Empty,
            (int)파밀리쥐 when form is "Family of Four" => "Four",

            (int)개굴닌자 or (int)암멍이 or (int)코라이돈 or (int)미라이돈 => string.Empty,

            _ => FormInfo.HasTotemForm(species) && form == "Large"
                ? species is (int)레트라 or (int)텅구리 ? "Alola-Totem" : "Totem"
                : form.Replace(' ', '-'),
        };
    }
    ...
    public static string SetShowdownFormName(ushort species, string form, int ability)
    {
        ...
        return species switch
        {
            (int)배쓰나이   when form == "Blue-Striped" => "Blue",
            (int)비비용   when form == "Pokeball"     => "Poké Ball",
            (int)네크로즈마   when form == "Dusk-Mane"    => "Dusk",
            (int)네크로즈마   when form == "Dawn-Wings"   => "Dawn",
            (int)스트린더 when form == "Low-Key"      => "Low Key",
            (int)불비달마 when form == "Galar-Zen"    => "Galar Zen",
            (int)메테노     when form != MiniorFormName => $"C-{form}",
            (int)지가르데    when form == "Complete"     => form,
            (int)지가르데    when ability == 211         => $"{(string.IsNullOrWhiteSpace(form) ? "50%" : "10%")}-C",
            (int)개굴닌자   when ability == 210         => "Ash", // Battle Bond
            (int)암멍이   when ability == 020         => "Dusk", // Rockruff-1
            (int)파밀리쥐   when form == "Four"         => "Family of Four",
            (int)우라오스 or (int)피카츄 or (int)마휘핑 => form.Replace('-', ' '), // Strike and Cosplay
